...
class _$ReportStateImpl implements _ReportState {
  const _$ReportStateImpl({this.status = Status.initial, this.user = User.A});
  ...
  @override
  String toString() {
    return 'ReportState(status: $status, user: $user)';
  }
...
abstract class _ReportState implements ReportState {
  const factory _ReportState({final Status status, final User user}) =
      _$ReportStateImpl;
